...
(async () => {
    ...
    try {
        ...
        // Extract potential buttons/links text - improved extraction
        const interactiveElements = await page.evaluate(() => {
            ...
            const ps = Array.from(document.querySelectorAll('p, span, div'))
                .filter(el => {
                    ...
                    return (text.includes('Consultar') || text.includes('Distonibilidad') || text.includes('Tiendas') || text.includes('Ubicación')) && getVisibility(el);
                })
                .map(d => ({
                    tag: d.tagName.toLowerCase(),
                    text: d.innerText.substring(0, 100).trim(),
                    class: d.className
                }));

            return [...buttons, ...links, ...ps];
        });
